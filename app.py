...
def home():
    ...
    return (
        ...
        f"/api/v1.0/tobs <br/>"
        f"/api/v1.0/start_dateYYYY-MM-DD/<start_date> <br/>" 
        f"/api/v1.0/start_end_dateYYYY-MM-DD/<start>/<end>"
...
def stations():
    ...
    results=[]
    for row in session.query(*sel).filter(Measurement.station == Station.station).group_by(Measurement.station).order_by(activity.desc()).all():
...
@app.route("/api/v1.0/start_dateYYYY-MM-DD/<start_date>")
...
@app.route("/api/v1.0/start_end_dateYYYY-MM-DD/<start>/<end>")
